...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    #( or: h = np.maximum(0, np.dot(X, W1) + b1)
    ...
    mask = -np.log(correct_scores_exp/np.sum(scores_exp, axis=1))
    loss = np.sum(mask)/N + reg * (np.sum(W1*W1) + np.sum(W2*W2))
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
    ...
    error2 = scores_exp/np.reshape(scores_exp_sum,(N,1))
    error2[range(N),y] -= 1
    error2 /= N
    dW2 = h.T.dot(error2)
    hdz = h
    hdz[hdz>0] = 1
    error1 = (error2.dot(W2.T))*hdz
    dW1 = X.T.dot(error1)
    grads['W1'] = dW1 + 2*reg*W1
    grads['W2'] = dW2 + 2*reg*W2
    grads['b1'] = np.sum(error1*hdz,axis=0)
